      ...
      if virtualization[:role] == 'host'
        ...
        %x{virsh list --all | grep -vP '\S*Id|^-|^$'}.each_line do |g|
          ...
          domains.each_line do |l|
            ...
            unless "#{id}" == "-"
              vncdisplay = %x(virsh vncdisplay #{id} 2> /dev/null).strip
              virtualization[:kvm][:guests][name][:vncdisplay] = sprintf("59%02d", vncdisplay.gsub(/(\d+.\d+.\d+.\d+)?:/, '')).to_i unless vncdisplay
            end
